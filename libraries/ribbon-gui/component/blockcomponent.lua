...
--IFN functions
function BlockComponent:mCalcSize(q, size, values)
    ...
    --TODO: Min/Max bounding on preferred size
	...
	q(function()
		...
		if not (self.attributes["location"] or self.attributes["dock"]) then
			...
			size.size:set(size.size:max(class.new(Size,
				self.position.x+self.size.width,
				self.position.y+self.size.height)))
		end
	end)
	self:queueChildrenCalcSize(q, msize, values)
end
...
function BlockComponent:updateContextPosition()
